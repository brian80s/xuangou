require 'test/unit'
require File.dirname(__FILE__) + '/../lib/extensions/standard_types'

class StandardTypesTest < Test::Unit::TestCase

  def test_commas
    #test float
    assert_equal "1,234,567.12",1234567.12.commas
    assert_equal "1,234,567.0",1234567.0.commas
    assert_equal "1,234,567.0",1234567.00.commas
    assert_equal "0.0",0.00.commas
    assert_equal "567.0",567.00.commas
    assert_equal "4,567.0",4567.00.commas

    #test fixnum
    assert_equal "1,234,567",1234567.commas
    assert_equal "1,234,567",1234567.commas
    assert_equal "1,234,567",1234567.commas
    assert_equal "0",0.commas
    assert_equal "567",567.commas
    assert_equal "4,567",4567.commas
    
    #test bignum
    assert_equal "11,111,111,111,111,111,111",11111111111111111111.commas
  end
  ...
  def test_time
    time=Time.mktime(2007,12,11,10,11)
    assert_equal "2007/12/11 10:11",time.to_long_datetime
    assert_equal "2007/12/11",time.to_short_datetime
    
    time=Time.mktime(2007,1,1,10,11)
    assert_equal "2007/01/01 10:11",time.to_long_datetime
    assert_equal "2007/01/01",time.to_short_datetime
  end
end
